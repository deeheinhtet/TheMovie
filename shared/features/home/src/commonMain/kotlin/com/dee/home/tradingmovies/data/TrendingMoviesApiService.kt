 ...
 */
class TrendingMoviesApiService(private val httpClient: HttpClient) {

    suspend fun fetchTradingMovies(request: MovieListRequest? = MovieListRequest(page = 1)): NetworkResult<MovieListResponse> {
        return httpClient.safeRequest<MovieListResponse> {
            url(PATH.plus(request?.trendingType?.param))
